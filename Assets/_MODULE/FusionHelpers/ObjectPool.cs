...
namespace FusionHelpers
{
	public class ObjectPool<T> : MonoBehaviour where T : MonoBehaviour
	{
		...
		public T AcquireInstance(T prefab, Vector3 pos = default, Quaternion rot = default, Transform parent = default)
		{
			...
			{
				Debug.Log($"Re-using pooled instance for prefab {prefab}");
		...
		public void ReleaseInstance(T networkObject)
		{
			if (networkObject)
			{
				ObjectList pool;
				if (_poolsByInstance.TryGetValue(networkObject, out pool))
				{
					pool.ReturnToPool(networkObject);
					networkObject.gameObject.SetActive(false); // Should always disable before re-parenting, or we will dirty it twice
